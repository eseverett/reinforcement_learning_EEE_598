...
def main() -> None:
    ...
    if run_tu:
        print('--- Vary Target Update ---')
        
        env.reset()
        replay_buffer.clear()
        
        target_update_list = [1, 50, 100, 500, 1000]

        for i in range(len(target_update_list)):
            ...
            training_cfg = {
                ...
                'environment': env,
                'device': device,
                'target_update_steps': target_update_list[i],
                'warmup_steps': 100,
                'gradient_steps_per_env_step': 1,
            }
        ...
        multi_plots(evaluator, agents, ep_returns_list, ep_losses_list, colors, 'Target Update Steps', target_update_list)
        agents.clear(); ep_returns_list.clear(); ep_losses_list.clear();
